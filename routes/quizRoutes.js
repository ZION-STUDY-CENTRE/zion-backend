...
router.put('/:id', authMiddleware, roleMiddleware('instructor', 'admin'), async (req, res) => {
  try {
    ...
    const { title, description, dueDate, scheduledDate, duration, passingMarks, questions, status } = req.body;
...
router.post('/:id/submit', authMiddleware, async (req, res) => {
  try {
    ...
    const percentageScore = (score / quiz.totalMarks) * 100;
...
router.get('/:id/submissions', authMiddleware, roleMiddleware('instructor', 'admin'), async (req, res) => {
  try {

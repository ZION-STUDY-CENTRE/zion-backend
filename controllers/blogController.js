...
exports.createBlogPost = async(req, res) => {
    try {
        ...
        const { type, title, url, platform, department, description, shortDescription, image } = req.body;
...
exports.updateBlogPost = async(req, res) => {
    try {
        ...
        post = await BlogPost.findByIdAndUpdate(req.params.id, { $set: updateData }, { new: true });
        res.json(post);
    } catch (err) {
        console.error(err.message);
        res.status(500).send('Server error');
